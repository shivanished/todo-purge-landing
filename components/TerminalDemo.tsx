    ...
    <pre className="font-mono text-[13px] leading-6 overflow-x-auto custom-scrollbar">
      {code.split("\n").map((line, i) => {
        ...
        return (
          <div
            key={i}
            className={`flex ${isTodoLine ? "bg-yellow-500/10" : ""}`}
          >
            <span className="w-8 shrink-0 text-slate-600 select-none text-right pr-4">
              {i + 1}
            </span>
              ...
              {isComment ? (
                ...
                <span className="text-slate-300">
                  {line.split(" ").map((word, wi) => {
                    ...
                    if (isImport && (word === "import" || word === "from"))
                      ...
                      isControl &&
                      ["if", "await", "return", "throw", "new", "else"].some(
                        (k) => word.startsWith(k)
                      )
                    )
                      color = "text-purple-400";
    ...
    if (view === "terminal") {
      const runSequence = async () => {
        ...
        const steps = [
          ...
          {
            msg: "Found 1 issue in src/controllers/dispenser.ts:14",
            delay: 600,
          },
      ...
      <div className="relative rounded-xl overflow-hidden shadow-2xl shadow-black/50 border border-[#2e2e2e] bg-[#0d0d0d] transition-all duration-500 h-[750px] flex flex-col font-sans">
        {view === "terminal" ? (
          ...
          <div className="flex flex-col h-full font-mono">
            {/* Terminal Header */}
            <div className="h-10 bg-[#1e1e1e] flex items-center px-4 border-b border-[#333] min-w-0">
              <div className="flex gap-2 shrink-0">
                <div className="w-3 h-3 rounded-full bg-[#ff5f56]"></div>
                <div className="w-3 h-3 rounded-full bg-[#ffbd2e]"></div>
                <div className="w-3 h-3 rounded-full bg-[#27c93f]"></div>
              </div>
              <div className="mx-auto text-gray-500 text-xs truncate min-w-0 max-w-full px-2">
                user@dev-machine: ~/projects/coffee-bot
              </div>
            </div>
            ...
            <div
              ref={terminalBodyRef}
              className="flex-1 bg-[#121212] p-6 text-sm text-gray-300 overflow-y-auto"
            >
              <div className="mb-2">
              ...
              <div className="flex">
                ...
                {typedCommand.length < 14 && (
                  <span className="animate-pulse bg-gray-500 w-2 h-4 ml-1 inline-block align-middle"></span>
                )}
              </div>
          ...
          <div className="flex h-full bg-[#131316] text-[#e5e5e5]">
            ...
            <div className="w-[240px] bg-[#191a23] border-r border-[#22232e] flex flex-col py-4 px-3 gap-6 shrink-0 hidden md:flex">
              ...
              <div className="space-y-0.5">
                <div className="text-[11px] font-semibold text-gray-500 px-2 mb-1 uppercase tracking-wider">
                  Your Space
                </div>
                <div className="flex items-center gap-2 px-2 py-1.5 bg-[#2a2b36] text-[#e5e5e5] rounded cursor-pointer text-sm font-medium">
                  <Inbox className="w-4 h-4" /> My Issues{" "}
            ...
            <div className="flex-1 flex flex-col min-w-0">
              {/* Toolbar */}
              <div className="h-12 border-b border-[#22232e] flex items-center px-6 justify-between shrink-0 min-w-0">
                <div className="flex items-center gap-2 text-gray-500 text-sm min-w-0 flex-1">
                  <span className="hover:text-gray-300 cursor-pointer truncate">
                    My issues
                  </span>
                  <span className="text-gray-700 shrink-0">/</span>
                  <span className="text-gray-300 truncate">{ticket.id}</span>
                </div>
                <div className="flex items-center gap-4 text-gray-500 shrink-0 min-w-0">
                  <Button
                    size="sm"
                    variant="ghost"
                    ...
                    <div className="bg-[#0f0f11] rounded-lg border border-[#2e2e2e] overflow-hidden mb-8 shadow-sm">
                      <div className="bg-[#1a1b23] px-4 py-2 text-xs font-mono text-gray-400 border-b border-[#2e2e2e] flex items-center gap-2">
                        ...
                        <span className="text-blue-400">
                          {ticket.codeContext?.line}
                        </span>
                      </div>
                      <div className="p-4">
                        <SyntaxHighlight
                          code={ticket.codeContext?.snippet || ""}
                        />
                      </div>
                    </div>
            ...
            <div className="w-[260px] border-l border-[#22232e] py-6 px-4 bg-[#191a23] hidden lg:block shrink-0">
              ...
              <div className="space-y-5">
                <div className="group">
                  <label className="text-xs text-gray-500 mb-1.5 block group-hover:text-gray-400 transition-colors">
                    Status
                  </label>
                ...
                <div className="group">
                  ...
                  <div className="flex flex-wrap gap-2 pt-1">
                    {ticket.labels.map((l) => (
                      <span
                        key={l}
                        className="text-xs bg-[#2a2b36] text-gray-300 px-2 py-0.5 rounded border border-[#383945] hover:border-gray-500 cursor-pointer transition-colors"
                      >
                        {l}
                      </span>
                    ))}
              ...
              <div className="mt-12 pt-6 border-t border-[#2e2e2e]">
                <div className="text-xs text-gray-500 mb-4">
                  Context from{" "}
                  <span className="text-brand-400">todo-purge</span>
                </div>
